...
POSE_DIM = 384  # Original pose dimension without counter
POSE_DIM_WITH_COUNTER = 385  # Pose + counter dimension
POSE_NORMALIZATION_SCALE = 100.0  # Divide coordinates by this value
PAD_TOKEN = "<pad>"
UNK_TOKEN = "<unk>"
BOS_TOKEN = "<bos>"
EOS_TOKEN = "<eos>"
...
def flatten_payload(payload: Any, center_square_crop: bool = True, normalize: bool = False) -> np.ndarray:
    """Convert one SignVerse DWPose frame payload to the StableSigner 384-vector order.

    Args:
        payload: DWPose payload dict
        center_square_crop: Apply center square crop transform
        normalize: Divide coordinates by POSE_NORMALIZATION_SCALE (StableSigner-style, optional)
    """
    ...
    # Normalize coordinates by dividing by POSE_NORMALIZATION_SCALE (e.g., 100.0)
    # This brings values from [0, 1] range to [0, 0.01] range, matching StableSigner's approach
    if normalize:
        body = body / POSE_NORMALIZATION_SCALE
        left_hand = left_hand / POSE_NORMALIZATION_SCALE
        right_hand = right_hand / POSE_NORMALIZATION_SCALE
        face = face / POSE_NORMALIZATION_SCALE
...
def vector_to_pose_dict(vec: np.ndarray, denormalize: bool = True) -> Dict[str, Any]:
    """Convert pose vector back to dict for rendering.

    Args:
        vec: Pose vector (384-dim or 385-dim with counter)
        denormalize: If True, multiply coordinates by POSE_NORMALIZATION_SCALE
    """
    vec = np.asarray(vec, dtype=np.float32).reshape(-1)
    # Remove counter if present
    if vec.shape[0] >= POSE_DIM_WITH_COUNTER:
        vec = vec[:POSE_DIM]
    if vec.shape[0] < POSE_DIM:
    ...
    # Denormalize coordinates if they were normalized during training
    if denormalize:
        body = body * POSE_NORMALIZATION_SCALE
        left_hand = left_hand * POSE_NORMALIZATION_SCALE
        right_hand = right_hand * POSE_NORMALIZATION_SCALE
    ...
    # Draw code expects normalized coordinates in [0, 1] and confidence-like scores.
...
class SignDWSegmentDataset(Dataset):
    ...
    def _load_pose(self, seg: Segment) -> Tuple[torch.Tensor, torch.Tensor, int]:
        """Load pose sequence with counter embedding.

        Returns pose frames with counter appended as last dimension.
        Counter is normalized frame index: (i-1) / total_frames for i in [1, total_frames]
        Following StableSigner's approach.
        """
        ...
        # Add counter embedding: (frame_index - 1) / total_frames
        # Counter is in [0, 1] range, where 0 is first frame, 1 is last frame
        counters = np.arange(1, true_len + 1, dtype=np.float32) / float(true_len)
        counters = counters.reshape(-1, 1)  # [true_len, 1]

        # Append counter to each frame: [POSE_DIM] -> [POSE_DIM + 1]
        frames_with_counter = np.concatenate([frames, counters], axis=1)  # [true_len, 385]

        mask = np.zeros((self.max_pose_frames,), dtype=np.float32)
        mask[:true_len] = 1.0

        if frames_with_counter.shape[0] < self.max_pose_frames:
            # Pad with zeros (counter=0 for padding frames)
            pad = np.zeros((self.max_pose_frames - frames_with_counter.shape[0], POSE_DIM_WITH_COUNTER), dtype=np.float32)
            frames_with_counter = np.concatenate([frames_with_counter, pad], axis=0)

        if self.normalize:
            if self.pose_mean is None or self.pose_std is None:
                raise ValueError("normalize=True requires pose_mean and pose_std")
            # Only normalize pose part, not counter
            frames_with_counter[:, :POSE_DIM] = (frames_with_counter[:, :POSE_DIM] - self.pose_mean) / self.pose_std

        return torch.from_numpy(frames_with_counter.astype(np.float32)), torch.from_numpy(mask), true_len
...
class TextToDWPoseTransformer(nn.Module):
    """Autoregressive text-to-pose transformer following StableSigner architecture.

    Key differences from original parallel decoder:
    - Uses target embedding layer instead of learnable queries
    - Autoregressive decoding: each frame attends to previous frames
    - Counter embedding provides explicit temporal guidance
    ...
    def __init__(
        ...
        ff_dim: int = 2048,
        dropout: float = 0.1,
        noise_rate: float = 0.0,  # Gaussian noise augmentation rate
    ) -> None:
        super().__init__()
        self.max_text_len = max_text_len
        self.max_pose_frames = max_pose_frames
        ...
        # Target (pose) embedding: project pose vectors to hidden_dim
        # Input: [batch, seq, POSE_DIM_WITH_COUNTER=385]
        # Output: [batch, seq, hidden_dim]
        self.target_embed = nn.Linear(POSE_DIM_WITH_COUNTER, hidden_dim)
        ...
        # Output head: predict POSE_DIM (384) - counter is kept from input
        self.pose_head = nn.Linear(hidden_dim, POSE_DIM)

        # Length prediction head (optional, for compatibility)
        ...
        # Xavier initialization (following StableSigner)
        self._init_weights()

        # For Gaussian noise augmentation
        self.register_buffer("out_stds", torch.ones(POSE_DIM_WITH_COUNTER), persistent=False)
    ...
    def update_noise_stats(self, pose_std: torch.Tensor) -> None:
        """Update noise standard deviations for Gaussian augmentation."""
        if pose_std is not None:
            self.out_stds = pose_std.to(self.out_stds.device)
    ...
    def forward(
        ...
        Args:
            text_ids: [batch, max_text_len] text token IDs
            target_poses: [batch, max_pose_frames, POSE_DIM_WITH_COUNTER] target poses (training only)
            target_mask: [batch, max_pose_frames] validity mask (training only)

        Returns:
            poses: [batch, max_pose_frames, POSE_DIM_WITH_COUNTER] predicted poses
            length_pred: [batch] predicted sequence length ratio
        """
        ...
        # Decode poses autoregressively
        if target_poses is not None:
            # Training mode: use teacher forcing
            poses = self._forward_train(memory, pad_mask, target_poses, target_mask)
        else:
            # Inference mode: autoregressive generation
            poses = self._forward_infer(memory, pad_mask)
    ...
    def _forward_train(
        ...
        """Training forward with teacher forcing and Gaussian noise augmentation.

        Args:
            memory: [batch, src_len, hidden_dim] encoded text
            memory_mask: [batch, src_len] padding mask for text
            target_poses: [batch, max_pose_frames, POSE_DIM_WITH_COUNTER] ground truth poses
            target_mask: [batch, max_pose_frames] validity mask

        Returns:
            [batch, max_pose_frames, POSE_DIM_WITH_COUNTER] predicted poses
        """
        ...
        if self.training and self.noise_rate > 0:
            ...
            noise[:, :, -1] = 0.0
            # Only add noise to valid frames
            if target_mask is not None:
                noise = noise * target_mask.unsqueeze(-1)
            decoder_input = decoder_input + self.noise_rate * noise

        # Embed target poses
        tgt = self.target_embed(decoder_input)  # [batch, seq, hidden_dim]
        tgt = tgt + self.pose_pos[:seq_len].unsqueeze(0)
        ...
        # Predict poses (384-dim, without counter)
        pred_poses = self.pose_head(y)  # [batch, seq, POSE_DIM]

        # Keep counter from input (ground truth counter during training)
        counter = target_poses[:, :, -1:]  # [batch, seq, 1]
        pred_with_counter = torch.cat([pred_poses, counter], dim=-1)  # [batch, seq, 385]

        return pred_with_counter

    def _forward_infer(self, memory: torch.Tensor, memory_mask: torch.Tensor) -> torch.Tensor:
        """Inference forward with autoregressive generation.
        ...
        # Initialize with zero pose + counter=0
        current_poses = torch.zeros(batch_size, 1, POSE_DIM_WITH_COUNTER, device=device)

        outputs = []

        for i in range(self.max_pose_frames):
            # Embed current sequence
            tgt = self.target_embed(current_poses)  # [batch, i+1, hidden_dim]
            tgt = tgt + self.pose_pos[: tgt.size(1)].unsqueeze(0)

            # Causal mask
            causal_mask = nn.Transformer.generate_square_subsequent_mask(tgt.size(1), device=device)
            ...
            # Predict next pose
            next_pose = self.pose_head(y[:, -1:, :])  # [batch, 1, POSE_DIM]

            # Add ground truth counter for next frame
            # Counter = (i+1) / max_pose_frames (following StableSigner)
            counter = torch.full((batch_size, 1, 1), (i + 1) / self.max_pose_frames, device=device)
            next_frame = torch.cat([next_pose, counter], dim=-1)  # [batch, 1, 385]

            outputs.append(next_frame)

            # Append to sequence for next iteration
            current_poses = torch.cat([current_poses, next_frame], dim=1)  # [batch, i+2, 385]

        # Stack all predictions
        result = torch.cat(outputs, dim=1)  # [batch, max_pose_frames, 385]
        return result


def masked_pose_loss(pred: torch.Tensor, target: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    """Compute masked pose loss (only on pose part, not counter).

    Args:
        pred: [batch, seq, POSE_DIM_WITH_COUNTER] predicted poses
        target: [batch, seq, POSE_DIM_WITH_COUNTER] ground truth poses
        mask: [batch, seq] validity mask
    ...
    # Only compute loss on pose part (first POSE_DIM dimensions), not counter
    pred_pose = pred[:, :, :POSE_DIM]  # [batch, seq, 384]
    target_pose = target[:, :, :POSE_DIM]  # [batch, seq, 384]

    loss = F.mse_loss(pred_pose * mask, target_pose * mask, reduction="sum")
    denom = mask.sum().clamp_min(1.0) * POSE_DIM
    return loss / denom
...
def evaluate(model: nn.Module, loader: DataLoader, device: torch.device) -> Dict[str, float]:
    ...
    for batch in loader:
        ...
        # Pass target_poses and target_mask for teacher forcing during evaluation
        pred, length_pred = model(text_ids, target_poses=poses, target_mask=pose_mask)
...
def load_raw_segment_poses(
    ...
    """Load raw pose frames for rendering/visualization.

    Returns pose vectors without counter embedding (384-dim).
    """
    with np.load(seg.npz_path, allow_pickle=True) as data:
        ...
        if max_frames > 0 and len(frame_indices) > max_frames:
            ...
            frame_indices = frame_indices[selected]
        return np.stack([flatten_payload(payloads[int(idx)], center_square_crop=center_square_crop, normalize=normalize) for idx in frame_indices], axis=0)
...
def predict_text(
    ...
    """Predict pose sequence for text using autoregressive generation.

    The model will generate poses autoregressively (one frame at a time) with
    ground truth counter values, following StableSigner's approach.
    """
    model.eval()
    ids, _ = vocab.encode(text, max_text_len)

    # Autoregressive inference: don't pass target_poses
    pred, length_pred = model(ids.unsqueeze(0).to(device))

    # pred shape: [1, max_pose_frames, POSE_DIM_WITH_COUNTER]
    pred_np = pred[0].detach().cpu().numpy()  # [max_pose_frames, 385]

    # Remove counter dimension for output
    pred_np = pred_np[:, :POSE_DIM]  # [max_pose_frames, 384]

    # Denormalize if z-score normalization was used
    pred_np = denormalize_pose(pred_np, pose_mean, pose_std)

    # Use predicted length to truncate
...
def compute_pose_stats(
    ...
    """Compute mean and std of pose data for z-score normalization.

    Args:
        segments: List of segments to sample from
        max_pose_frames: Max frames per segment
        sample_limit: Max number of segments to sample
        center_square_crop: Apply center square crop
        normalize: Apply StableSigner-style normalization (divide by 100)

    Returns:
        (mean, std) arrays of shape (POSE_DIM,)
    """
    ...
    for seg in sampled:
        ...
        if len(frame_indices) > max_pose_frames:
            ...
            frame_indices = frame_indices[selected]
        frames = np.stack([flatten_payload(payloads[int(idx)], center_square_crop=center_square_crop, normalize=normalize) for idx in frame_indices], axis=0)
...
def load_checkpoint(path: Path, device: torch.device) -> Tuple[TextToDWPoseTransformer, Vocabulary, Dict[str, Any], Optional[np.ndarray], Optional[np.ndarray]]:
    ...
    model = TextToDWPoseTransformer(
        ...
        noise_rate=float(cfg.get("noise_rate", 0.0)),  # Default to 0.0 for old checkpoints
    ).to(device)
...
def train(args: argparse.Namespace) -> None:
    ...
    for step in range(1, args.max_steps + 1):
        ...
        with torch.cuda.amp.autocast(enabled=args.amp and device.type == "cuda"):
            # Pass target_poses and target_mask for teacher forcing during training
            pred, length_pred = model(text_ids, target_poses=poses, target_mask=pose_mask)
            pose_loss = masked_pose_loss(pred, poses, pose_mask)
            length_loss = F.mse_loss(length_pred, length_target)
